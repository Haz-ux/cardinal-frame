...
import { reinforcePattern, skillNameFromPhrases, uniqueSkillName, sanitizeSkillName } from '../learn.mjs';
 ...
 *   audit, broadcast, logger, DATA_DIR, callAgentLLM
 */
export default function toolsRoutes(ctx) {
  const { db, stmts, authMiddleware, optionalAuth, requireRole, apiLimiter, audit, broadcast, logger, DATA_DIR, callAgentLLM } = ctx;
  ...
  router.post('/learn/run-loop', authMiddleware, requireRole('admin'), apiLimiter, async (_req, res) => {
    const result = await runLearnLoop({ db, stmts, randomUUID, broadcast, audit, logger, callAgentLLM });
    if (!result) {
      const patterns = stmts.patterns.getAll.all();
  ...
  router.post('/skills/auto-propose', authMiddleware, requireRole('admin'), apiLimiter, async (req, res) => {
    ...
    const bestObservations = intentGroups[bestIntent];
    const phrases = bestObservations.map(o => o.user_input);

    // Name the skill after what the user keeps asking to do. Aimi (the LLM)
    // proposes a cleaner name when available; otherwise we derive it from the
    // most recurring phrase deterministically.
    let skillName = null;
    if (callAgentLLM) {
      try {
        const sample = phrases.slice(0, 8).map((p, i) => `${i + 1}. "${String(p).slice(0, 120)}"`).join('\n');
        const result = await callAgentLLM([
          { role: 'system', content: 'You are Aimi, naming a reusable skill for Cardinal Frame. Based on the recurring user requests below, reply with ONLY a concise human-readable skill name, 2-5 words, no quotes, no punctuation, no "skill" suffix, no "auto-" prefix. Example: for recurring "check system health" requests reply "Check System Health".' },
          { role: 'user', content: sample },
        ]);
        skillName = sanitizeSkillName(result?.content);
      } catch { /* fall through to deterministic name */ }
    }
    if (!skillName) {
      skillName = skillNameFromPhrases(phrases, { maxWords: 5 }) || `Auto ${bestIntent} Skill`;
    }
    skillName = uniqueSkillName(stmts, skillName);

    const skillDesc = `Auto-proposed skill for the recurring request "${phrases[0] || bestIntent}" (from ${bestCount} observations)`;
    const skillHandler = `async (input) => {\n  // Auto-generated by Aimi self-learning\n  // Based on ${bestCount} observations: ${phrases[0] || bestIntent}\n  return { handled: true, intent: '${bestIntent}' };\n}`;
