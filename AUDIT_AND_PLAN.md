...
| Area | Item | Priority |
|------|------|----------|
| — | No open items. All previously tracked findings are resolved (see below). | — |
...
| Auth | JWT was long-lived (24h) with no refresh or logout. Now: short-lived access token (`JWT_EXPIRES`, default `15m`) + opaque, SHA-256-hashed, rotatable refresh token (`JWT_REFRESH_EXPIRES`, default `7d`) in a `refresh_tokens` table. `POST /api/auth/refresh` rotates the pair; `POST /api/auth/logout` revokes server-side. Client auto-refreshes on 401. Migration `012_refresh_tokens.sql`. | Medium |
| Docs | `ARCHITECTURE.md` rate-limit and sandbox figures aligned with reality (write API 50/min, read 200/min, auth 20/min, sandbox 10/min; skill VM sandbox 30s default / 1 MB buffer; `POST /api/sandbox/execute` 5s / 100 KB). | Low |
| Schema | `skill_hub_sources.type` CHECK constraint expanded to `('git','github','url','tarball','http')` to match route usage (`github`/`url`). Rebuilt via migration `013_skill_hub_source_types.sql`; routes also validate `type` against an allowlist. | Low |
