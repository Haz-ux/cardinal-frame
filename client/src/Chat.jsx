 ...
 const messagesEndRef = useRef(null);
  ...
  const handleAgentSend = useCallback(async () => {
    ...
    try {
      ...
      const planMsg = {
        role: 'assistant',
        content: `**Aimi Plan** (${agentMode === 'agent' ? 'Agent' : 'Suggest'} mode)\n\n` +
        ...
        if (step.action === 'read') {
          ...
          try {
            const result = await api('/api/agent/write', {
              ...
              body: JSON.stringify({
                path: step.target,
                content: step.content || `// Generated by Aimi for: ${step.description}\n// No content provided for this step.\n`,
                scope: 'sandbox',
  ...
  return (
    <div style={{ display: 'flex', height: '100%', minHeight: 0 }}>
      ...
      <Database size={13} /> State
        ...
        placeholder={
          agentMode === 'agent' ? `Aimi will plan + execute autonomously...` :
          agentMode === 'suggest' ? `Aimi will draft changes for your approval...` :
