...
export default function Files() {
 ...
 useEffect(() => { load(); }, []);
  ...
  const deleteFile = async (id, name) => {
    ...
    await api(`/api/files/${id}`, { method: 'DELETE' }); load();
    if (previewName === id) { setPreviewName(null); setPreviewContent(''); }
  };
  const previewFile = async (id, name) => {
    if (previewName === id) { setPreviewName(null); setPreviewContent(''); return; }
    try {
      const resp = await fetch(`/api/files/${id}/download`);
      const text = await resp.text();
      setPreviewName(id);
      setPreviewContent(text.slice(0, 5000));
    } catch { setPreviewName(id); setPreviewContent('[Preview unavailable]'); }
  };
     ...
     <div className="rounded-xl overflow-hidden" style={{ background: 'rgba(10,10,20,0.9)', border: '1px solid rgba(0,240,255,0.1)' }}>
       {filtered.length === 0 ? (
         <div className="text-center py-12" style={{ color: '#444' }}><Folder size={32} className="mx-auto mb-2 opacity-30" />No files uploaded.</div>
          ...
          const fname = file.original_name || file.name || '(unnamed)';
          return (
          <div key={file.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.03)' }}>
            <div className="flex items-center gap-3 px-4 py-2.5 transition-colors cursor-pointer"


             onClick={() => previewFile(file.id, fname)}>
             {getFileIcon(fname)}
             <span className="flex-1 text-sm truncate" style={{ color: '#ddd' }}>{fname}</span>
             <span className="text-xs hidden sm:block" style={{ color: '#555' }}>{formatSize(file.size)}</span>
             <span className="text-[10px] font-mono hidden md:block" style={{ color: '#444' }}>{file.uploaded_at ? new Date(file.uploaded_at + 'Z').toLocaleDateString() : '—'}</span>
             <button onClick={(e) => { e.stopPropagation(); previewFile(file.id, fname); }} className="p-1 rounded transition" style={{ color: '#555' }}
>
               <Eye size={14} />
             </button>
             <button onClick={(e) => { e.stopPropagation(); deleteFile(file.id, fname); }} className="p-1 rounded transition" style={{ color: '#555' }}
>
               <Trash2 size={14} />
             </button>
            </div>
